...
class ACVNavigationController: UINavigationController {
    ...
    open override func pushViewController(_ viewController: UIViewController, animated: Bool) {
        ...
        if viewControllers.count > 1 {
            ...
            var backImg = ACVNavigationAppearance.appearance.backImage
    ...
    func backAction() {
        ...
        ACVNavigationAppearance.appearance.globalBackHandler?()
        popViewController(animated: true)
    }
    /// 获取控制器的代理对象
    private var navigationBarUI: ACVNavigationBarUIDelegate? {
        let topViewController = topViewController as? ACVNavigationBarUIDelegate
...
// MARK: 手势处理
extension ACVNavigationController: UIGestureRecognizerDelegate {
...
// MARK: 判断只有一个视图手势禁止
extension ACVNavigationController: UINavigationControllerDelegate {
    public func navigationController(_ navigationController: UINavigationController, didShow viewController: UIViewController, animated: Bool) {
        ...
        if viewControllers.count > 1 {
            if let tabbar = self.tabBarController as? ACVTabBarViewController {
