...
/// 基类方便后期做扩展
open class ACVBaseViewController: UIViewController {
...
// MARK: 设置默认的导航栏
@objc extension ACVBaseViewController: ACVNavigationBarUIDelegate {
    /// 是否允许手势进行返回
    open func shouldBeginInteractivePopGesture(_ gesture: UIGestureRecognizer) ->  Bool { true }
    /// 设置 titleView 的 tintColor
    open var lkrTitleViewTintColor: UIColor? { ACVNavigationAppearance.appearance.titleColor }
    /// 设置 titleView 的 font
    open var lkrTitleViewFont: UIFont? { ACVNavigationAppearance.appearance.titleFont }
    /// 设置导航栏的背景图，默认为 NavBarBackgroundImage
    open var lkrNavigationBarBackgroundImage: UIImage? {
        let appearance = ACVNavigationAppearance.appearance
    ...
    open var lkrBackIndicatorImage: UIImage? {
        ACVNavigationAppearance.appearance.backImage
    }
}
